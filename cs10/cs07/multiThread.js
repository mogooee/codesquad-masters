const { Process, workerThread, Worker } = require("./process");
...
class ProcessScheduling {
  ...
  makeJobQueue() {
    if (workerThread.isMainThread) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (
          this.jobQueue.queue.filter(
            (process) => process.pcb.time === randomNum
          ).length == 0
        ) {
  ...
  dispatch() {
    const process = this.readyQueue.queue[0];
    return new Promise((resolve, reject) => {
      for (let worker of process.threads) {
